    ...
    public function authenticate(Request $request)
    {
        $validator = Validator::make($request->all(),
            [
                ...
                'email.required' => 'Employee Id or Email is required',
        ...
        if ($credentials['is_email']) {
            ...
            $input = ['empID' => $credentials['email'], 'password' => $credentials['password'], 'status_id' => 1];
        }


        if (Auth::attempt($input)) {
            if (Auth::user()->role->web_login != 1) {
                Auth::logout();
                return response()->json(['error' => ['email' => ["You don't have permission to web login"]]], 422);
            }
            return 0;
        } else {
