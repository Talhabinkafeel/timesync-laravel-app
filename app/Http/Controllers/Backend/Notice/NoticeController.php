...
class NoticeController extends Controller
{
    ...
    {
      try{
        $data['title']=_trans('notice.Notice Statistics');
        $data['fields']=$this->notice->statisticsField();
        $data['notice']=$this->notice->show($id);
        $data['statistics']=$this->notice->statistics($id)->original['data'];
        return view('backend.notice.statistics', compact('data'));
      }catch(\Exception $e){
        dd($e->getMessage());
      }
    }
    ...
    public function store(NoticeReqeust $request)
    {
        ...
        try {
            if ($this->isExistsWhenStore($this->model, 'subject', $request->subject)) {
                ...
                Toastr::success(_trans('response.Notice created successfully'), 'Success');
    ...
    public function update(NoticeReqeust $request, Notice $notice)
    {
        ...
        try {
            if ($this->isExistsWhenUpdate($notice, $this->model, 'subject', $request->subject)) {
                ...
                Toastr::success(_trans('response.Notice updated successfully'), 'Success');
                return redirect()->route('notice.index');
            } else {
                Toastr::error("{$request->subject} already exists", 'Error');
                return redirect()->back();
    ...
    public function delete(Notice $notice): \Illuminate\Http\RedirectResponse
    {
        ...
        try {
            ...
            Toastr::success(_trans('response.Notice deleted successfully'), 'Success');
            return redirect()->back();
        } catch (\Exception $e) {
            Toastr::error(_trans('response.Something went wrong!'), 'Error');
            return redirect()->back();
