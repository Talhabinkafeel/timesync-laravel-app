...
class AttendanceReportRepository
{
        ...
        return datatables()->of($monthArray)
            ...
            ->addColumn('checkin', function ($data) use ($user) {
                ...
                if (in_array($todayDateName, $weekEnds)) {
                    return '<span class="badge badge-primary ml-2" data-toggle="tooltip" data-placement="top"> Weekend </span>';
                }
                ...
                if ($holidays != '') {
                    return '<span class="badge badge-success ml-2" data-toggle="tooltip" data-placement="top"> Holiday </span>';
                }
                ...
                if ($leaveDate != '') {
                    return '<span class="badge badge-warning ml-2" data-toggle="tooltip" data-placement="top"> Leave </span>';
                }
            ...
            ->addColumn('checkout', function ($data) use ($user) {
                ...
                $attendance = $this->attendance->query()->where(['user_id' => $user->id, 'date' => $day->format('Y-m-d')])->orderBy('id', 'desc')->first();
                ...
                if ($attendance) {
                    if($attendance->check_out) {
                        if ($attendance->out_status == AttendanceStatus::LEFT_TIMELY) {
                            ...
                            $status = '';
                            $status .= '<span class="badge badge-primary">' . $this->dateTimeInAmPm($attendance->check_out) . '</span>';
                            $status .= '<span class="badge badge-primary ml-2" data-toggle="tooltip" data-placement="top" title="' . $attendance->check_out_location . '"> <i class="fa fa-h-square"></i> </span>';
                            return $status;
                        }
                    }else{
            ...
            ->addColumn('in_status', function ($data) use ($user) {
                $day = Carbon::parse($data);
                $attendance = $this->attendance->query()->where(['user_id' => $user->id, 'date' => $day->format('Y-m-d')])->first();
                if ($attendance) {
                    $intime = abs($attendance->late_time);
                    $hours = floor($intime / 60); // Calculate the number of hours
                    $minutes = $intime % 60; // Calculate the remaining minutes
                    $isNegative = ($intime < 0);
                    ...
                    if ($attendance->in_status == AttendanceStatus::ON_TIME) {
                        $status = '';
                        $status .= '<span class="badge badge-success">On Time'." ". $totalTime.'</span>';
                       // $status .= '<span class="badge badge-primary badge-sm ml-2">'.$attendance->check_in_location.' </span>';
                        return $status;
                    } elseif ($attendance->in_status == AttendanceStatus::LATE) {
                        $status = '';
                        $status .= '<span class="badge badge-danger">Late'." ". $totalTime.'</span>';
                        $status .= '<br><span class="">'.@$attendance->lateInReason->reason.' </span>';
                        return $status;
                    }
                } else {
                    return '';
            ...
            ->addColumn('out_status', function ($data) use ($user) {
                $day = Carbon::parse($data);
                $attendance = $this->attendance->query()->where(['user_id' => $user->id, 'date' => $day->format('Y-m-d')])->orderBy('id', 'desc')->first();
                if ($attendance) {
                    if($attendance->check_out) {
                        if ($attendance->out_status == AttendanceStatus::LEFT_TIMELY) {
                            $status = '';
                            $status .= '<span class="badge badge-success">Left Timely</span>';
                           // $status .= '<span class="badge badge-primary ml-2" data-toggle="tooltip" data-placement="top" title="' . $attendance->check_out_location . '"> <i class="fa fa-h-square"></i> </span>';
                            return $status;
                        } elseif ($attendance->out_status == AttendanceStatus::LEFT_EARLY) {
                            $totalSeconds = 0;
                            $dutySchedule = Shift::where('id', $user->shift_id)->with('dutySchedule')->first();
                            $earltoutime = '';
                            $sctime = explode(' ',$attendance->check_out);
                            if(strtotime($sctime[1]) < strtotime($dutySchedule->dutySchedule->end_time)){
                                    $endTime = strtotime($sctime[1]);
                                    $totalSeconds += strtotime($dutySchedule->dutySchedule->end_time) - $endTime;
                            }
                            ...
                            $status = '';
                            $status .= '<span class="badge badge-danger">Left Early'." ".$earltoutime .'</span>';
                            // $status .= '<span class="badge badge-primary ml-2" data-toggle="tooltip" data-placement="top" title="' . $attendance->check_out_location . '"> <i class="fa fa-h-square"></i> </span>';
                            $status .= '<br><span class="">'. @$attendance->earlyOutReason->reason . ' </span>';
                            return $status;
                        } elseif ($attendance->out_status == AttendanceStatus::LEFT_LATER) {
                            $status = '';
                            $status .= '<span class="badge badge-primary">Left Later</span>';
                            //$status .= '<span class="badge badge-primary ml-2" data-toggle="tooltip" data-placement="top" title="' . $attendance->check_out_location . '"> <i class="fa fa-h-square"></i> </span>';
                            return $status;
                        }
                    }else{
            ...
            ->addColumn('hours', function ($data) use ($user) {
                ...
                $attendance = $this->attendance->query()->where(['user_id' => $user->id, 'date' => $day->format('Y-m-d')])->get();
                ...
                if ($attendance) {
                    ...
                    foreach($attendance as $key=>$att) {
                        if ($att->check_out) {
                            $startTime = strtotime($att->check_in);
                            $endTime = strtotime($att->check_out);
                            $totalSeconds += $endTime - $startTime; // add seconds to total seconds
                        }
                    }
                    $hours = floor($totalSeconds / 3600);
                    $minutes = floor(($totalSeconds / 60) % 60);
                    $totalTime = $hours.' hrs '.$minutes.' mins'; // format total hours and minutes as a string
					
                    if ($totalSeconds > 0) { // check if total time worked is greater than 0
                        return $totalTime;
                    } else {
                        return '';
                    }
            ...
            ->addColumn('overtime', function ($data) use ($user) {
                ...
                if ($attendance) {
                    ...
                    if ($attendance->check_out) {
                        ...
                        if(strtotime($sctime[1]) > strtotime($dutySchedule->dutySchedule->end_time)){
                                $endTime = strtotime($sctime[1]);
                            $totalSeconds += $endTime - strtotime($dutySchedule->dutySchedule->end_time);
                        }
                    }
                    // $workingHour = @$dutySchedule->dutySchedule->hour;
                    //$totalOverTime = ($totalSeconds) - $workingHour*3600;
            ...
            ->rawColumns(array('date', 'checkin', 'checkout','in_status','out_status', 'totalBreak', 'breakDuration', 'hours', 'overtime'))
            ->make(true);
    }
    ...
    public function singleAttendanceSummary($user, $request)
    {
        ...
        foreach ($monthArray as $day) {
            ...
            $attendances = $this->attendance->query()->where(['company_id' => $this->companyInformation()->id, 'user_id' => $user->id, 'date' => $todayDateInSqlFormat])->get();
            ...
            if ($attendance) {
                ...
                $hours = floor($allseconds / 3600);
                $minutes = floor(($allseconds / 60) % 60);
                $totalWorkTime = $hours.' hrs '.$minutes.' mins';
     ...
     try {
           ...
           if ($request->search) {
               $attendance = $attendance->whereHas('user', function ($query) use ($request) {
                   $query->where('empID', 'like', '%' . $request->search . '%');
               });
           }
           if ($request->user_id) {
           ...
           return [
               'data' => $data->map(function ($data) {
                   ...
                   return [
                       ...
                       'checkout'      => $this->checkOutTable($data) ?? '<span class="badge badge-danger">Absent</span>',
                       'hours'        => (@$data->check_out) ? $this->timeDifference($data->check_in, $data->check_out) : '',
                       'in_status'   => $this->in_status($data),
                       'out_status'  => $this->out_status($data) ?? '',
                       'overtime'     => $this->NewoverTimeCount($data) ?? '',
                       'date'       =>  $data->date,
    ...
    // Check In Status
    public function in_status($data){
        if ($data) {
            $intime = abs($data->late_time);
            $hours = floor($intime / 60); // Calculate the number of hours
            $minutes = $intime % 60; // Calculate the remaining minutes
            $isNegative = ($intime < 0);
            if ($hours == 0 && $minutes == 0) {
            ...
            if ($data->in_status == AttendanceStatus::ON_TIME) {
                $status = '';
                $status .= '<span class="badge badge-success">On Time'." ". $totalTime.'</span>';
               // $status .= '<span class="badge badge-primary badge-sm ml-2">'.$attendance->check_in_location.' </span>';
                return $status;
            } elseif ($data->in_status == AttendanceStatus::LATE) {
                $status = '';
                $status .= '<span class="badge badge-danger">Late'." ". $totalTime.'</span>';
                $status .= '<br><span class="">'.@$data->lateInReason->reason.' </span>';
                return $status;
            }
        } else {
            return '';
    ...
    // Checkout Status
    public function out_status($data){
        if ($data) {
            if($data->check_out) {
                if ($data->out_status == AttendanceStatus::LEFT_TIMELY) {
                    $status = '';
                    $status .= '<span class="badge badge-success">Left Timely</span>';
                   // $status .= '<span class="badge badge-primary ml-2" data-toggle="tooltip" data-placement="top" title="' . $attendance->check_out_location . '"> <i class="fa fa-h-square"></i> </span>';
                    return $status;
                } elseif ($data->out_status == AttendanceStatus::LEFT_EARLY) {
                    $totalSeconds = 0;
                    $user = User::find($data->user_id);
                    $dutySchedule = Shift::where('id', $user->shift_id)->with('dutySchedule')->first();
                    $earltoutime = '';
                    $sctime = explode(' ',$data->check_out);
                    if(strtotime($sctime[1]) < strtotime($dutySchedule->dutySchedule->end_time)){
                            $endTime = strtotime($sctime[1]);
                            $totalSeconds += strtotime($dutySchedule->dutySchedule->end_time) - $endTime;
                    }
                    ...
                    $status .= '<span class="badge badge-danger">Left Early'." ".$earltoutime .'</span>';
                    // $status .= '<span class="badge badge-primary ml-2" data-toggle="tooltip" data-placement="top" title="' . $attendance->check_out_location . '"> <i class="fa fa-h-square"></i> </span>';
                    $status .= '<br><span class="">'. @$data->earlyOutReason->reason . ' </span>';
                    return $status;
                } elseif ($data->out_status == AttendanceStatus::LEFT_LATER) {
                    $status = '';
                    $status .= '<span class="badge badge-primary">Left Later</span>';
                    //$status .= '<span class="badge badge-primary ml-2" data-toggle="tooltip" data-placement="top" title="' . $attendance->check_out_location . '"> <i class="fa fa-h-square"></i> </span>';
                    return $status;
    ...
    // New Overtime Count
    public function NewoverTimeCount($data){
        if ($data) {
            $endTime = 0;
            $totalSeconds = 0;
            $user = User::find($data->user_id);
    ...
    // Detailed Attendance Table
     function PresentAttentanceTable($request)
    {
     ...
     try {
		...
		$subquery = DB::table('attendances')
			->select(DB::raw('user_id, date(check_in) as checkin_date, min(check_in) as first_checkin'))
			->where('company_id', auth()->user()->company_id)
			->whereNotNull('check_in')
			->where('in_status', 'OT')
			->groupBy('user_id', 'checkin_date');

			$attendance = $attendance->joinSub($subquery, 'sub', function($join) {
				$join->on('attendances.user_id', '=', 'sub.user_id')
					 ->whereRaw('date(attendances.check_in) = sub.checkin_date')
					 ->whereRaw('attendances.check_in = sub.first_checkin');
			});
		 ...
		 return [
               'data' => $data->map(function ($data) {
                    // $status = $this->markattendance($data);
                   $action_button = '';
                   if (hasPermission('attendance_update')) {
                       $action_button .= actionButton(_trans('common.Edit'), route('attendance.checkInEdit', $data->id), 'profile');
                   }
                   $button = ' <div class="dropdown dropdown-action">
                    ...
                    if($data->in_status === 'OT'){
                        return [
                            ...
                            // 'totalBreak'       =>  RawTable('employee_breaks')->where(['date' => $data->date, 'user_id' => $data->user_id])->count(),
                            // 'breakDuration'  => breakDuration($data->date, $data->user_id),
                            'checkin'      => '<span class="badge badge-success">' . $this->dateTimeInAmPm($data->check_in) . '</span>',
    ...
    /function markattendance($data){
        ...
        //Check if checkin and checkout are on time then present
        if($checkInStatus[0] === 'OT'){
    ...
    */
    ...
    function LateAttentanceTable($request)
    {
               ...
               'data' => $data->map(function ($data) {
                   ...
                   $button = ' <div class="dropdown dropdown-action">
                    ...
                    return [
                            ...
                            // 'totalBreak'       =>  RawTable('employee_breaks')->where(['date' => $data->date, 'user_id' => $data->user_id])->count(),
                            // 'breakDuration'  => breakDuration($data->date, $data->user_id),
                            'checkin'      => '<span class="badge badge-warning">' . $this->dateTimeInAmPm($data->check_in) . '</span>',
                            // 'checkout'      => $this->checkOutTable($data) ?? '',
                            // 'hours'        => (@$data->check_out) ? $this->timeDifference($data->check_in, $data->check_out) : '',
                            // 'overtime'     => $this->overTimeCount($data) ?? '',
                            'date'       =>  $data->date,
    ...
    function AbsentAttentanceTable($request)
    {

     try {
        
            // Get all users in the company with their attendance records
            ...
            if ($request->search) {
                $users = $users->where('empID', 'like', '%' . $request->search . '%');
            }
            ...
            $data = $users->paginate($request->limit ?? 2);
            return [
                'data' => $data->map(function ($user) use ($request) {
                    ...
                    //$attendance = $user->attendances()->whereDate('date', $todayDateInSqlFormat)->first();
                    ...
                    //if (!$attendance) {
                        ...
                        // Check if today is a weekend day for the company
                        $weekEnds = Weekend::where(['company_id' => $user->company->id, 'is_weekend' => 'yes'])->pluck('name')->toArray();
                        $todayDateName = Str::lower($todayDateName);
                        
                        $isWeekend = in_array($todayDateName, $weekEnds);
                        
            
                        // Determine the user's status based on leave, holiday, or weekend
                        if ($leaveDate) {
                            $status = '<span class="badge badge-warning">Leave</span>';
                        } elseif ($holidays) {
                            $status = '<span class="badge badge-success">Holiday</span>';
                        } elseif ($isWeekend) {
                            $status = '<span class="badge badge-primary">WeekEnd</span>';
                        } else {
                   ...
                   // }
                    ...
                    $action_button = '';
                    if (hasPermission('attendance_update')) {
                        $action_button .= actionButton(_trans('common.Edit'), route('attendance.checkInEdit', $user->id), 'profile');
                    }
                    $button = ' <div class="dropdown dropdown-action">
                                        <button type="button" class="btn-dropdown" data-bs-toggle="dropdown"
                                            aria-expanded="false">
                    ...
                    return [
                        'id' => $user->id,
                        'name' => $user->name? '<a href="' . route('employeeAttendance', @$user->id) . '" target="_blank">' . @$user->name . '</a>':'',
						'empID' => $user->empID,
                        'department' => @$user->department->title,
                        'date' => $todayDateInSqlFormat,
                        'status' =>  $status,
                        'action' => $button,
                    ];
                }),
    ...
    // NoCheckout Attendance Table
    function NoCheckoutAttentanceTable($request)
    {
