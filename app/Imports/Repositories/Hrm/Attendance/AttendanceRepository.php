...
class AttendanceRepository
{
    ...
    public function userAttendance($user, $request)
    {
        if ($user) {
            ...
            if ($isIpRestricted) {
                ...
                $attendance_status = $this->checkInStatus($user->id, $request->check_in);
    ...
    public function checkOut($request, $id)
    {
        ...
        try {
            ...
            if ($user) {
                ...
                if ($isIpRestricted) {
                    $request['checkin_ip'] = getUserIpAddr();
                    $attendance_status = $this->checkOutStatus(Auth::id(), $request->check_out);
                    if (count($attendance_status) > 0) {
    ...
    function fields()
    {
        return [
            ...
            _trans('attendance.Check In Status'),
            _trans('attendance.Check Out Status'),
            _trans('common.Hours'),
    ...
    function present_report_fields()
    {
        return [
            _trans('common.ID'),
            _trans('common.Name'),
    ...
    // Absent Report Fields
    function absent_report_fields()
    {
        return [
            _trans('common.ID'),
            _trans('common.Name'),
    ...
    // NoCheckout Report Fields
    function nocheckout_report_fields()
    {
        return [
            _trans('common.ID'),
            _trans('common.Name'),
    ...
    public function employe_attendance($request){
        ...
        $validator = Validator::make($request->all(),
            ...
            [
               'empID.required' => 'Empoloyee ID is required',
               'startDate.required' => 'Starting Date is required',
               'endDate.required' => 'Ending Date is required', 
               'startDate.date' => 'Must be type date',
               'endDate.date' => 'Must be type date',
            ]);
        ...
        $user = User::where('empID', $request->empID)
                    ->whereHas('attendances', function ($query) use ($request) {
                        $query->whereBetween('check_in', [start_end_datetime($request->startDate, $request->endDate)]);
                    })
        ...
        if ($user && $user->attendances) {
            foreach ($user->attendances as $attendance) {
                $attendanceData = [
                    ...
                    'check_in_status' => $attendance->in_status,
                    'check_out_status' => $attendance->out_status,
                    'check_in_location' => $attendance->check_in_location,
                    'check_out_location' => $attendance->check_out_location,
                    'Hours' => $this->worktime($attendance),
                    'Overtime' => $this->overtimecount($attendance,$user),
                ];
        ...
        if($data){
            return $this->responseWithSuccess('Employee Attendance Record', $data, 200);
        }else{
            return $this->responseWithError('Record Not Found', $data, 400);
        }
    }

    //WorkTime Count
    public function worktime($attendance){
        $totalSeconds = 0;
        if($attendance->check_out){
    ...
    //OverTime Count
    public function overtimecount($attendance,$user){
