...
class LeaveRequestRepository
{
                    ...
                    if ($leaveType->department_id == $userDepartment->department_id) {
                        ...
                        if ($user) {
                            ...
                            if ($leaveRequest->user->myHr() != null) {
                                $this->sendFirebaseNotification($leaveRequest->user->myHr()->id, 'leave_approved', $leaveRequest->id, route('leaveRequest.index'));
    ...
    public function staffListView($request)
    {
       ...
       try {
            ...
            $my_team_members=auth()->user()->myTeam()->pluck('id')->toArray();

            if ($my_team_members) {
                $leaveRequests->whereIn('user_id', $my_team_members);
            }
    ...
    public function approveRejectLeave($request)
    {
        try{
            ...
            if ($leaveRequest) {
                if($leaveRequest->user->manager && Auth::id()==$leaveRequest->user->manager->id){
                    if ($request->leave_status == 'approved') {
                ...
                }else if($leaveRequest->user->myHr() && Auth::id()==$leaveRequest->user->myHr()->id){
                    if ($request->leave_status == 'approved') {
