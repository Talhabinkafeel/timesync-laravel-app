...
class NoticeRepository
{
    ...
    public function noticeList($request)
    {
        ...
        $notices = $this->notice->query()
            ...
            ->when($request->month, function ($query) use ($request) {
                return $query->where('date','LIKE','%'.$request->month.'%');
            })
    ...
    public function currentMonthNotice(){
        try {
            $notices = $this->notice->query()
            ->where('notices.company_id', $this->companyInformation()->id)
            ->Join('notice_departments', 'notice_departments.noticeable_id', '=', 'notices.id')
    ...
    public function show($id): \Illuminate\Http\JsonResponse
    {
        ...
        //Read Notice Notification
        $noticeViewLog = Notification::where(['notifiable_id' => auth()->id(),'notification_for'=>'notice', 'id_for' => $id])->first();

        if ($noticeViewLog) {
            $noticeViewLog->read_at=Carbon::now();
            $noticeViewLog->save();
        }
    ...
    public function statistics($id)
    {
      ...
      try {
        ...
        $notice_seen=Notification::with('receiver','receiver.department','receiver.designation')->where(['notification_for'=>'notice', 'id_for' => $id])->where('read_at','!=',null)->get();
        $notice_unseen=Notification::with('receiver','receiver.department','receiver.designation')->where(['notification_for'=>'notice', 'id_for' => $id])->where('read_at',null)->get();

        $seen=$notice_seen->map(function($item){
            return [
                'id'=>$item->receiver->id,
                'name'=>$item->receiver->name,
                'avatar'=>$item->receiver->avatar_id ? uploaded_asset($item->receiver->avatar_id) : null,
                'department'=>$item->receiver->department->title,
                'designation'=>$item->receiver->designation->title,
                'read_at'=>$item->read_at,
            ];
        });
        $unseen=$notice_unseen->map(function($item){
            return [
                'id'=>$item->receiver->id,
                'name'=>$item->receiver->name,
                'avatar'=>$item->receiver->avatar_id ? uploaded_asset($item->receiver->avatar_id) : null,
        ...
        return $this->responseWithSuccess('Notice Statistics', $data, 200);
      } catch (\Throwable $th) {
    ...
    public function store($request): \Illuminate\Http\JsonResponse
    {
        ...
        try {
            ...
            //check 0 in department id
            if (in_array(0, $request->department_id)) {
                $users = User::where('company_id', $this->companyInformation()->id)->get();
                foreach ($users as $user) {
                    ...
                    $this->sendCustomNotification($user, $details,$notification_for);
                    // sendDatabaseNotification($user, $details,$notification_for);

                    $this->sendCustomFirebaseNotification($user->id, 'notice', '', 'notice', $request->subject, $request->description);
                }
                $departments=Department::where('company_id', $this->companyInformation()->id)->get();
                foreach ($departments as $department) {
                    $departmentNotice = new NoticeDepartment;
                    $departmentNotice->department_id = $department->id;
    ...
    public function storeNotice($request)
    {
        ...
        try {
            ...
            foreach ($request->department_id as $item) {
                $department = Department::with('users')->where('id', $item)->where('company_id', Auth::user()->company_id)->first();
                if($department && $department->has('users')){
                    $this->sendChannelFirebaseNotification($department->title.$department->id, 'notice', '', 'notice', $request->subject, $request->description, $notice->attachment_file_id?uploaded_asset($notice->attachment_file_id):null);
                }else{
                    return $this->responseWithError("Department has no user. Please check again.", [], 500);
                }
        ...
        return datatables()->of($notice->latest()->get())
            ...
            ->addColumn('action', function ($data) {
                ...
                if (hasPermission('designation_delete')) {
                    $action_button .= actionButton('Delete', '__globalDelete(' . $data->id . ',`dashboard/announcement/notice/delete/`)', 'delete');
                }
    ...
    function table($request)
    {
        ...
        return [
            'data' => $data->map(function ($data) {
                ...
                if (hasPermission('notice_update')) {
                    $action_button .= '<a href="' . route('notice.statistics', $data->id) . '" class="dropdown-item"> Statistics</a>';
                }
                if (hasPermission('designation_delete')) {
                    $action_button .= actionButton('Delete', '__globalDelete(' . $data->id . ',`dashboard/announcement/notice/delete/`)', 'delete');
                }
