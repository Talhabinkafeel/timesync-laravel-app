...
class UserRepository
{
    ...
    public function save($request)
    {
        ...
        try {
            ...
            $request['password'] = Hash::make($request->password == '' ? 12345678 : $request->password);
    ...
    { {
            ...
            $data =  $this->model->query()->where('company_id', $this->companyInformation()->id)->where('branch_id', userBranch())
                ->select('id', 'company_id', 'role_id', 'department_id','manager_id', 'designation_id', 'name', 'email', 'empID', 'phone', 'status_id', 'shift_id', 'is_free_location', 'is_hr','is_admin')
                ->where('company_id', auth()->user()->company_id);
            
            $where = array();
            
            if (@$request->search) {
                $where[] = ['empID', 'like', '%' . $request->search . '%'];
            }
