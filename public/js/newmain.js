...
$(document).ready(function () {
  ...
  if (themeMode) {
    $("body").removeClass(themeMode);
    $("body").addClass(themeMode);
    $('input[name="theme_mode"]').each(function () {
      if ($(this).val() == themeMode) {
    ...
    if (themeMode == "dark-theme") {      
      ...
      $(".dark_logo").addClass("d-none");
    } else {
      $("body").attr("style", "background-image: url("+ $('#background_image').val() +")");
      $(".white_logo").addClass("d-none");
      $(".dark_logo").removeClass("d-none");
    }
  } else {
    $("body").attr("style", "background-image: url("+ $('#background_image').val() +")");
  ...
  $('input[name="theme_mode"]').on("change", function () {
    ...
    localStorage.setItem("theme_mode", $(this).val());
    $("body").removeClass("default-theme dark-theme");
    $("body").addClass($(this).val());

    if ($(this).val() == "dark-theme") {
      $(".white_logo").removeClass("d-none");
      $(".dark_logo").addClass("d-none");
      $("body").removeAttr("style");
    } else {      
  ...
  $(".sidebar-dropdown-menu li a").each(function () {
    ...
    if ($this.attr("href") == current) {
      ...
      // scroll to selected menu item end
    }
  });
  ...
  $(".item-content button").on("click", function () {
    ...
    setTimeout(function () {
      ...
      if($(thisBtn).siblings(".dropdown-menu").length){
        menuPosition = $(thisBtn).siblings(".dropdown-menu").offset().top;
      }

      for (let i = 0; i < headerHeight - menuPosition; i++) {
        $(".header .dropdown-menu").css("top", headerHeight - menuPosition);
        $(".header .dropdown-menu").css("transition", "top .4s");
      }
    }, 200);
  });
...
*/
  ...
  // currency and lanuage select start
  $(".dropdown-item-list select").each(function () {
    let $this = $(this),
    ...
    let styledSelectImg = $this.children("option").eq(0).attr("image-data");
    let styledSelectRText = $this.children("option").eq(0).text();

    $styledSelect.html(
      `${
        styledSelectImg ? `<img src="${styledSelectImg}"/>` : ""
      }<span>${styledSelectRText}</span>`
    );
    ...
    for (let i = 0; i < numberOfOptions; i++) {
      let img = $this.children("option").eq(i).attr("image-data");
      let rel = $this.children("option").eq(i).val();
      let text = $this.children("option").eq(i).text();

      $("<li />", {
        html: `${img ? `<img src="${img}"/>` : ""}<span>${text}</span>`,
        rel,
      }).appendTo($list);
    }
  ...
  // currency and lanuage select end
